...
- [ ] 4.1 Agent基础架构
  - [ ] 4.1.1 实现基于LangChain的Agent核心
  - [ ] 4.1.2 配置OpenRouter LLM集成
  - [ ] 4.1.3 实现会话管理和上下文保持
- [ ] 4.2 工具系统开发
  - [ ] 4.2.1 实现RAG检索工具
    - [ ] 4.2.1.1 文档检索功能
    ...
    - [ ] 4.2.1.3 检索结果排序和筛选
  - [ ] 4.2.2 实现知识图谱查询工具
    - [ ] 4.2.2.1 先修关系查询（PREREQUISITE：当前→下一步学什么）
    - [ ] 4.2.2.2 推荐内容生成（基于COVERS关系推荐对应作业）
    - [ ] 4.2.2.3 概念关联查询（基于TEACHES/PRACTICES关系）
    - [ ] 4.2.2.4 学习路径验证（确保推荐不跳过先修）
- [ ] 4.3 Skill系统开发
  - [ ] 4.3.1 实现Skill加载和管理机制
  - [ ] 4.3.2 实现Skill触发和选择逻辑
  - [ ] 4.3.3 实现工具与Skill的绑定机制
...
- [ ] 5.1 答疑Skill开发
  ...
  - [ ] 5.1.3 实现基于RAG的答案生成
  - [ ] 5.1.4 实现引用格式化功能
  - [ ] 5.1.5 添加苏格拉底式追问支持（可选）
- [ ] 5.2 推荐练习Skill开发
  - [ ] 5.2.1 创建推荐Skill配置文件
  - [ ] 5.2.2 实现基于知识图谱的推荐算法
    - [ ] 5.2.2.1 基础推荐：按PREREQUISITE关系推荐下一讲
    - [ ] 5.2.2.2 练习推荐：按COVERS关系推荐对应作业
    - [ ] 5.2.2.3 概念推荐：基于DEPENDS_ON关系推荐相关概念（可选）
  - [ ] 5.2.3 实现推荐理由生成
  - [ ] 5.2.4 实现个性化推荐调整
- [ ] 5.3 Skill协同机制
  - [ ] 5.3.1 实现多Skill同时触发的处理逻辑
  - [ ] 5.3.2 实现合并回答策略（先答疑后推荐）
  - [ ] 5.3.3 实现Skill结果合并和格式化
  - [ ] 5.3.4 添加Skill使用状态跟踪
